...
public class OrderServiceImpl implements OrderService {
    ...
    public Boolean sava(String[] ids, BookstoreOrder order) {
        ...
        while (true) {
            BookstoreOrder bookstoreOrder = orderMapper.getOne(randomLong);
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            //获取当前id的购物车记录
            BookstoreShoppingCart shoppingCart = shoppingCartMapper.getOne(Long.valueOf(ids[i]));
            //创建订单关系表，并保持
            ...
            orderShoppingCartMapper.save(orderShoppingCart);

            //获取book数据
            Long bookId = shoppingCart.getTbBookstoreShoppingCartBookId();
            BookstoreBook book = bookMapper.getOne(bookId);
            //如果书籍信息不存在，将cart中的书籍信息持久化到order_book
            OrderBook orderBook = toOrderBook(book);
            OrderBook rest = orderBookMapper.getOne(orderBook.getId());
            if (rest == null) {
                orderBookMapper.save(orderBook);
            }
            //当前购买的数量
            ...
            //删除出当前购物车中id为当前值得记录
            shoppingCartMapper.deleteById(Long.valueOf(ids[i]));
        }
        return null;
    }

    @Override
    public Map listOrderDetailById(Long id, Long userId, int pageNum, int pageSize) {
